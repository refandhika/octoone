...
trait UploadableWidget
{
    ...
    public function onUpload()
    {
        ...
        try {
            ...
            if (!File::exists(public_path() . '/public/storage')) {
                File::makeDirectory(public_path() . '/public/storage', 0755, true);
            }
            ...
            Storage::disk('public_media')->put($fileName, File::get($realPath), 'public');

            $s3Path = 'storage/app/media/' . $fileName;
            Storage::disk('s3')->delete($s3Path);
            Storage::disk('s3')->put($s3Path, File::get($realPath), 'public');
