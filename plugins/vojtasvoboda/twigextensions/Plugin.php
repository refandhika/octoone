...
class Plugin extends PluginBase
{
    ...
    private function getStringLoaderFunctions($twig)
    {
        ...
        return [
            'template_from_string' => function ($template) use ($twig) {
    ...
    private function getTextFilters($twig)
    {
        ...
        $twig->addExtension($textExtension);

        return [
            'truncate' => function ($value, $length = 30, $preserve = false, $separator = '...') use ($twig) {
            ...
            'wordwrap' => function ($value, $length = 80, $separator = "\n", $preserve = false) use ($twig) {
                return twig_wordwrap_filter($twig, $value, $length, $separator, $preserve);
            }
        ];
    }
    ...
    private function getLocalizedFilters($twig)
    {
        ...
        return [
            'localizeddate' => function ($date, $dateFormat = 'medium', $timeFormat = 'medium', $locale = null, $timezone = null, $format = null) use ($twig) {
                return twig_localized_date_filter($twig, $date, $dateFormat, $timeFormat, $locale, $timezone, $format);
            },
            'localizednumber' => function ($number, $style = 'decimal', $type = 'default', $locale = null) use ($twig) {
                return twig_localized_number_filter($number, $style, $type, $locale);
            },
            'localizedcurrency' => function ($number, $currency = null, $locale = null) use ($twig) {
    ...
    private function getTimeFilters($twig)
    {
        return [
            'time_diff' => function ($date, $now = null) {
                ...
                $formattedDiff = $diff->format('%y years, %m months, %d days, %h hours, %i minutes, %s seconds');
