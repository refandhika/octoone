...
class Index extends Controller
{
    ...
    public function onSave()
    {
        ...
        $result = [
            'objectPath' => 'content' != $type ? $object->getBaseFileName() : $object->fileName,
    ...
    protected function addPageSyntaxFields($formWidget, $page)
    {
        ...
        foreach ($fields as $fieldCode => $fieldConfig) {
            if ('fileupload' == $fieldConfig['type']) {
                continue;
            }

            if ('repeater' == $fieldConfig['type']) {
    ...
    protected function fillObjectFromPost($type)
    {
        ...
        if ('page' == $type) {
            ...
            if (is_array($placeholders) && true === Config::get('cms.convertLineEndings', false)) {
        ...
        if ('content' == $type) {
            ...
            if ('static-pages' == dirname($fileName)) {
                throw new ApplicationException(trans('rainlab.pages::lang.content.cant_save_to_dir'));
            }

            $extension = pathinfo($fileName, PATHINFO_EXTENSION);

            if ('htm' === $extension || 'html' === $extension || !strlen($extension)) {
                $objectData['markup'] = array_get($saveData, 'markup_html');
            }
        }

        if (!empty($objectData['markup']) && true === Config::get('cms.convertLineEndings', false)) {
