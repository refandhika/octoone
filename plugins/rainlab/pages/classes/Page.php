...
class Page extends ContentBase
{
    ...
    {
        if (false !== $this->processedMarkupCache) {
    ...
    public static function resolveMenuItem($item, $url, $theme)
    {
        ...
        if ('static-page' == $item->type && !isset($tree[$item->reference])) {
            return;
        }

        $result = [];

        if ('static-page' == $item->type) {
        ...
        if ($item->nesting || 'all-static-pages' == $item->type) {
            ...
            $result['items'] = $iterator('static-page' == $item->type ? $pageInfo['items'] : $tree['--root-pages--']);
        }
