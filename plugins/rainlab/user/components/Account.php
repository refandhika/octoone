...
use RainLab\User\Models\Throttle as UserThrottle;
...
class Account extends ComponentBase
{
    ...
    {
        return [''=>'- refresh page -', '0' => '- no redirect -'] + Page::sortBy('baseFileName')->pluck('baseFileName', 'baseFileName');
    }
    ...
    public function onSignin()
    {
        try {
            ...
            if ($check_user = Auth::findUserByLogin($credentials['login'])) {
                $user_throttle = Auth::findThrottleByUserId($check_user->id);
                if($user_throttle->checkSuspended()){
                    $msg = 'Your account has been suspeded. Please try again in a few minutes.';
                    if (Request::ajax()){
            ...
            //if (Request::segment(1) == 'indomaret' && Request::segment(2) == 'login') {
            if (Request::segment(1) == 'aktivitas-kami' && Request::segment(2) == 'promo-indomaret' && Request::segment(3) == 'login') {
                return Redirect::to('aktivitas-kami/promo-indomaret/main');
            }
            ...
            if (Request::ajax()){
                return response('The username and/or password are incorrect. Please try again.', 500);
            }
            else{
                Flash::error("The username and/or password are incorrect. Please try again.");
            }
        }
    }
    ...
    {
        try {
            // return Redirect::to('/indomaret/main');

            //if (!(Request::segment(1) == 'indomaret' && Request::segment(2) == 'daftar')) {
            if (!(Request::segment(1) == 'aktivitas-kami' && Request::segment(2) == 'promo-indomaret' && Request::segment(3) == 'daftar')) {
                $code = post('code', $code);
    ...
    {   
        $rules = [
            'telp' => 'max:255|regex:/^[0-9-. ]+$/u',
            'tgl_lahir' => 'max:255|regex:/^[0-9- ]+$/u',
            'alamat' => 'max:255|regex:/^[A-Za-z0-9- ,.!@#$%^&*()]+$/u',
            'email' => 'email',
            'jk' => 'max:255|regex:/^[A-Za-z0-9- ,.!@#$%^&*()]+$/u'
        ];
    ...
    {
        $rules = [
            'emails' => 'required|email|between:6,255'
        ];
        ...
        $user = UserModel::findByEmail(post('emails'));
    ...
    protected function makeResetUrl($code)
    {
        ...
        if (strpos($url, $code) === false) {
            $url .= url()->full() . '/forget-form?reset=' . $code;
        }

        return $url;
    }
	
	public function onSendEmailSwiss()
    {
		...
		$user = array("name"=>post('emails'),"email"=>post('emails'));
        ...
        //Flash::success('Check your email.');
		try{
        Mail::send('rainlab.user::mail.swissemail', $data, function($message) use ($user) {
			$message->subject("SWISS CHOCO FF");
            $message->to($user["email"], $user["name"]);
        });
		Flash::success('SEND.');
		//echo "send";
