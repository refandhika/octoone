...
class RecordList extends ComponentBase
{
    ...
    {
        $pages = Page::sortBy('baseFileName')->pluck('baseFileName', 'baseFileName');
    ...
    protected function paginate($model)
    {
        ...
        $modelClassName = $this->property('modelClass');
        if(str_contains($modelClassName, 'Pressrelease\Models\PressRelease')) {
            $records = $model->where('published_status','Yes')->paginate($recordsPerPage);
        } else {
            $records = $model->paginate($recordsPerPage);
        }
